...
function Bundles() {
  ...
  return (
    <section>
      <h2>Bundles</h2>
      <ul>{bundles.map((b) => <li key={b.slug}><strong>{b.name}</strong> — {b.description}</li>)}</ul>
    </section>
  );
}
...
function Workflows() {
  ...
  return (
    <section>
      <h2>Workflows</h2>
      <ul>{workflows.map((w) => <li key={w.slug}><strong>{w.name}</strong> — {w.description}</li>)}</ul>
    </section>
  );
}
